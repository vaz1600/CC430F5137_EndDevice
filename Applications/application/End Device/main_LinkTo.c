...
void main (void)
{
...
#if 0
  // после перезагрузки пытаемс€ подключитьс€ к точке доступа/другому устройству
  ...
  // запустить таймер сна на 1 секунду
  // в нем отсчитывать нужное врем€ (интервал замера температуры 15 минут)
  // включение датчика bmp, ожидание его замеров ( лучше несколько)
  // пробуждение трансивера
  // передача данных на точку доступа
  // еще надо напр€жение питани€ мер€ть свое
  while(1)
  {
      //for()
      BSP_TURN_ON_LED1();
      ...
      switch(radio_state)
      {
          ...
          case RADIO_LINKED:
                // делаем три попытки передать данные
                for(i = 0; i < 3; i++)
                {
                    BSP_TURN_ON_LED3();
                    //в случае отсутстви€ ответа/подтверждени€ засыпаем на 1 секунду
...
static void linkTo()
{
  ...
  while (1)
  {
    ...
    // считываем температуру раз в минуту
    temp = bme280_readTemperature();
    pressure = bme280_readPressure();
    hum = bme280_readHumidity();
